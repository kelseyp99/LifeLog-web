...
export const ExpertDataDialog: React.FC<Props> = ({ open, onClose, tokenDoc, expertName, clientName }) => {
  ...
  useEffect(() => {
    ...
    getDocs(collection(db, `Users/${tokenDoc.userId}/ActivityLog`))
      .then((snap: any) => setRows(snap.docs.map((d: any) => ({ ...d.data(), id: d.id }))))
      .catch(() => setRows([]))
      .finally(() => setLoading(false));
    if (tokenDoc?.id) {
      getDocs(collection(db, `ExpertNotes/${tokenDoc.id}/notes`))
        .then((snap: any) => setSavedNotes(snap.docs.map((d: any) => ({ id: d.id, ...d.data() }))))
        .catch(() => setSavedNotes([]));
    }
  }, [open, tokenDoc]);

  const categories = Array.from(new Set(rows.map((r: ActivityRow) => r.category).filter(Boolean))) as string[];

  const filtered = rows.filter((r: ActivityRow) => {
  ...
  const handleSort = (k: string) => { if (sortKey === k) setSortAsc((a: boolean) => !a); else { setSortKey(k); setSortAsc(true); } };
  ...
  const handleAI = async () => {
    ...
    try {
      const questionNote = tokenDoc?.question ? `\n\nClient's question: "${tokenDoc.question}"` : '';
      const text = filtered.map((r: ActivityRow) => `[${r.category}] ${r.description || ''} (${tsOf(r)?.toLocaleDateString() ?? ''})`).join('\n');
      const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
        ...
        body: JSON.stringify({ model, messages: [
          ...
          { role: 'user', content: `Activity log:${questionNote}\n\n${text}` }
        ]}),
      });
      ...
      <div style={{ background: '#fff', borderRadius: 14, width: '92vw', maxWidth: 900, maxHeight: '90vh', display: 'flex', flexDirection: 'column', boxShadow: '0 8px 40px rgba(0,0,0,0.25)' }}>
        ...
        <div style={{ background: '#ebf8ff', borderRadius: '14px 14px 0 0', padding: '18px 24px 0', borderBottom: '2px solid #bee3f8' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 }}>
            <div style={{ flex: 1 }}>
              ...
              {/* Client's question — shown prominently */}
              {tokenDoc?.question && (
                <div style={{ marginTop: 12, background: '#fffbeb', border: '2px solid #f6e05e', borderRadius: 10, padding: '12px 16px', maxWidth: 580 }}>
                  <div style={{ fontSize: 11, fontWeight: 800, color: '#744210', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 4 }}>💬 Client's Question</div>
                  <div style={{ fontSize: 15, color: '#2d3748', fontStyle: 'italic', lineHeight: 1.5 }}>"{tokenDoc.question}"</div>
                </div>
              )}
            </div>
            <button onClick={onClose} style={{ background: 'none', border: 'none', fontSize: 26, cursor: 'pointer', color: '#718096', lineHeight: 1, marginLeft: 12 }}>✕</button>
          </div>
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: 20 }}>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                ...
                <tbody>
                  {filtered.length === 0
                    ...
                    : filtered.map((r: ActivityRow, i: number) => (
          ...
          {!loading && tab === 'ai' && (
            <div>
              {tokenDoc?.question && (
                <div style={{ background: '#fffbeb', border: '1px solid #f6e05e', borderRadius: 8, padding: '10px 14px', marginBottom: 14, fontSize: 14, color: '#744210' }}>
                  <strong>💬 Client asks:</strong> "{tokenDoc.question}"
                </div>
              )}
          ...
          {!loading && tab === 'advice' && (
            <div>
              {tokenDoc?.question && (
                <div style={{ background: '#fffbeb', border: '1px solid #f6e05e', borderRadius: 8, padding: '10px 14px', marginBottom: 16, fontSize: 14, color: '#744210' }}>
                  <strong>💬 Client's question:</strong> "{tokenDoc.question}"
                </div>
              )}
              <div style={{ background: '#fffbeb', border: '1px solid #f6e05e', borderRadius: 10, padding: 16, marginBottom: 20, fontSize: 14, color: '#744210' }}>
                ...
                Write a message that will appear directly in <strong>{displayName}</strong>'s Activity Log. Free — no charge.
              </div>
              <div style={{ display: 'flex', gap: 8, marginBottom: 12, alignItems: 'center' }}>
                <label style={{ fontWeight: 600, color: '#4a5568', fontSize: 14, whiteSpace: 'nowrap' }}>Category:</label>
                <input value={adviceCategory} onChange={e => setAdviceCategory(e.target.value)} placeholder="Expert Advice"
                  style={{ padding: '7px 12px', borderRadius: 6, border: '1px solid #cbd5e1', fontSize: 14, flex: 1 }} />
              </div>
              <textarea value={adviceText} onChange={e => setAdviceText(e.target.value)} rows={6}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <button onClick={handlePostAdvice} disabled={adviceSaving || !adviceText.trim()}
